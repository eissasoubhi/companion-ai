...
function parseChannel(value: unknown, field: 'local' | 'remote'): CaptureChannelEvidence {
  ...
  if (
    !Number.isInteger(value.startupLatencyMs) ||
    (value.startupLatencyMs as number) < 0 ||
    (value.startupLatencyMs as number) > MAX_STARTUP_LATENCY_MS
  ) {
    throw new Error(
      `capture.${field}.startupLatencyMs must be an integer between 0 and ${MAX_STARTUP_LATENCY_MS}.`,
    );
  }
  ...
  return {
    ...
    liveTrack: true,
    signalDetected: true,
    startupLatencyMs: value.startupLatencyMs as number,
  };
}
...
export function parseCaptureValidationEvidence(value: unknown): CaptureValidationEvidence {
  ...
  if (value.schemaVersion !== CAPTURE_VALIDATION_SCHEMA_VERSION) {
    throw new Error(`schemaVersion must be ${CAPTURE_VALIDATION_SCHEMA_VERSION}.`);
  }

  if (value.rawAudioPersisted !== false) {
    throw new Error('rawAudioPersisted must be false.');
  }

  if (!isRecord(value.platform) || !hasOnlyKeys(value.platform, ['os', 'version', 'arch'])) {
    throw new Error('platform has an invalid shape.');
  }

  if (value.platform.os !== 'macos') {
    throw new Error('platform.os must be macos for the P0 capture validation.');
  }

  if (value.platform.arch !== 'arm64' && value.platform.arch !== 'x64') {
    throw new Error('platform.arch must be arm64 or x64.');
  }

  if (!isRecord(value.app) || !hasOnlyKeys(value.app, ['target', 'version'])) {
    throw new Error('app has an invalid shape.');
  }

  if (!CAPTURE_VALIDATION_TARGETS.includes(value.app.target as CaptureValidationTarget)) {
    throw new Error('app.target is not a supported validation target.');
  }

  if (!isRecord(value.capture) || !hasOnlyKeys(value.capture, ['local', 'remote'])) {
    throw new Error('capture has an invalid shape.');
  }

  const appVersion =
    value.app.version === undefined ? undefined : requireString(value.app.version, 'app.version');
  const notes =
    value.notes === undefined ? undefined : requireString(value.notes, 'notes', MAX_NOTES_LENGTH);
  ...
  return {
    ...
    platform: {
      os: 'macos',
      version: requireString(value.platform.version, 'platform.version'),
      arch: value.platform.arch,
    },
    app: {
      target: value.app.target as CaptureValidationTarget,
      ...(appVersion === undefined ? {} : { version: appVersion }),
    },
    capture: {
      local: parseChannel(value.capture.local, 'local'),
      remote: parseChannel(value.capture.remote, 'remote'),
    },
    rawAudioPersisted: false,
...
export function buildCaptureValidationMatrix(values: readonly unknown[]): CaptureValidationMatrix {
  ...
  for (const value of values) {
    ...
    if (evidence.has(parsed.app.target)) {
      throw new Error(`Duplicate capture validation target: ${parsed.app.target}.`);
    }
